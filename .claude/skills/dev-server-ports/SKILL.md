...
description: |
  このリポジトリのフロントエンド(Vite)・バックエンド(Hono)開発サーバーを
  起動・再起動するときに必ず使う。npm run dev / vite を実行する前、または
  ポート5173・8080が「already in use」「Port 5173 is already in use」
  「EADDRINUSE」等のエラーで競合したときに読むこと。
...
- バックエンド(Hono): `8080`
  - `backend/` 作成時に固定する。環境変数などで別ポートに切り替えない。
    待ち受けは `127.0.0.1` のみ(認証を持たないため外部に公開しない)。

## なぜ固定するか

フロントの `vite.config.ts` は `/api` を `http://127.0.0.1:8080` へプロキシしている（バックエンドは `127.0.0.1` のみ待ち受けで、`localhost` が `::1` に解決されると繋がらないため）。
